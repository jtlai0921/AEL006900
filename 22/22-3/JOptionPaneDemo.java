...
public class JOptionPaneDemo extends JFrame implements ActionListener {
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    String title = "<html><P><font color=black>對話框所顯示的訊息，可為任意形式的物件<br>例如<font color=red><b>HTML格式</b></font>.</P></html>";

    Object message[] = new Object[2];
    
    message[0] = "<html><P><font color=black>對話框所顯示的訊息，可為任意形式的物件<br>例如<font color=red><b>HTML格式或其他Swing物件</b></font>.</P></html>";

    GraphicsEnvironment ge = GraphicsEnvironment.getLocalGraphicsEnvironment();
    String font[] = ge.getAvailableFontFamilyNames();

    JComboBox cboFont = new JComboBox();
    cboFont.setPreferredSize(new Dimension(150, 36));
    cboFont.setMaximumSize(new Dimension(150, 36));
    ...
    // 
      ...
      joptionpane.showMessageDialog(
        null, title, "Message Dialog", JOptionPane.ERROR_MESSAGE);
        
      jlabel.setText("訊息對話框無回傳值");
    }
    else if (e.getSource().equals(jrbmenuitem[2][1])) { // 一般訊息 
      joptionpane.showMessageDialog(
        null, title, "Message Dialog", JOptionPane.INFORMATION_MESSAGE);
        
      jlabel.setText("訊息對話框無回傳值");
    }
    else if (e.getSource().equals(jrbmenuitem[2][2])) { // 純文字訊息
      joptionpane.showMessageDialog(
        null, title, "Message Dialog", JOptionPane.PLAIN_MESSAGE);
        
      jlabel.setText("訊息對話框無回傳值");
    }
    else if (e.getSource().equals(jrbmenuitem[2][3])) { // 詢問訊息
      joptionpane.showMessageDialog(
        null, title, "Message Dialog", JOptionPane.QUESTION_MESSAGE);
        
      jlabel.setText("訊息對話框無回傳值");
    }
    else if (e.getSource().equals(jrbmenuitem[2][4])) { // 警告訊息
      joptionpane.showMessageDialog( 
        null, title, "Message Dialog", JOptionPane.WARNING_MESSAGE);
