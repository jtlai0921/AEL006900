...
public class JScrollBarDemo extends JFrame implements AdjustmentListener {
  ...
  public JScrollBarDemo() {
    super("DefaultBoundedRangeModel Demo");
    ...
    DefaultBoundedRangeModel model[] = new DefaultBoundedRangeModel[3];
    
    for (int i=0; i<3; i++){
      model[i] = new DefaultBoundedRangeModel(0, 0, 0, 255);
    }
    
    // red
    jscrollbar1 = new JScrollBar(SwingConstants.HORIZONTAL);
    // 設定JScrollBar類別的MVC架構中Model部份。
    jscrollbar1.setModel(model[0]);
    jscrollbar1.setPreferredSize(new Dimension(200, 20));
    // 註冊 AdjustmentListener
    jscrollbar1.addAdjustmentListener(this);
    
    // green
    jscrollbar2 = new JScrollBar(SwingConstants.HORIZONTAL);
    // 設定JScrollBar類別的MVC架構中Model部份。
    jscrollbar2.setModel(model[1]);
    jscrollbar2.setPreferredSize(new Dimension(200, 20));
    // 註冊 AdjustmentListener
    jscrollbar2.addAdjustmentListener(this);
    
    // blue
    jscrollbar3 = new JScrollBar(SwingConstants.HORIZONTAL);
    // 設定JScrollBar類別的MVC架構中Model部份。
    jscrollbar3.setModel(model[2]);
