...
public class ClipboardServiceDemo extends javax.swing.JFrame {
  ...
  private PrintService prs = null;
  private ClipboardService cbs = null; 
  ...
  public static void main(String[] args) {
    ...
    new ClipboardServiceDemo();
  }

  // 建構函式
  public ClipboardServiceDemo() {
    super("JNLP ClipboardService");
    ...
    // Cut
    JMenuItem jmnuCut1 = new JMenuItem(item[6], new ImageIcon(cl.getResource("images/" + image[6] + ".gif")));
    ...
    // Copy
    JMenuItem jmnuCopy1 = new JMenuItem(item[7], new ImageIcon(cl.getResource("images/" + image[7] + ".gif")));
    ...
    // Paste
    JMenuItem jmnuPaste1 = new JMenuItem(item[8], new ImageIcon(cl.getResource("images/" + image[8] + ".gif")));
  ...
  private void print(JTextPane txtContent) {
    final JTextPane printPane = txtContent;
    ...
    new Thread() {
      public void run() {
        if (prs == null) {
          try {
            // 查詢PrintService服務
            prs = (PrintService)ServiceManager.lookup("javax.jnlp.PrintService");
          } 
          catch(UnavailableServiceException e) {
            // 無法查詢到指定服務，回傳UnavailableServiceException
            prs = null;
        ...
        if (prs != null) {
          try {
            prs.print(new DocToPrinter(printPane));
          } 
          catch(Exception ex) {
            ex.printStackTrace();
          }
  ...
  private void pageSetup() {
    ...
    if (prs != null) {
      try {
        // 以預設之版面設定建立PageFormat物件，以供列印之用
        java.awt.print.PageFormat pf = prs.getDefaultPage(); 
        
        // 顯示版面設定對話盒以設定邊界、列印方向、紙張大小、紙張來源、印表機等
        java.awt.print.PageFormat newPf = prs.showPageFormatDialog(pf); 
      } 
      catch(Exception ex) {
        ex.printStackTrace();
      }
  ...
  private void toClipboard(String content) {
    ...
    if (cbs != null) {
      java.awt.datatransfer.StringSelection ss = new StringSelection(content); 
  ...
  private String fromClipboard() {
    ...
    if (cbs != null) {
      java.awt.datatransfer.Transferable tf = cbs.getContents();
  ...
  private void menu_actionPerformed(ActionEvent e) {
    ...
    else if (e.getActionCommand().equals(item[4])) { // print
      print(txtContent);
    }
    else if (e.getActionCommand().equals(item[5])) { // page setup
      pageSetup();
    }
    else if (e.getActionCommand().equals(item[6])) { // cut
      toClipboard(txtContent.getSelectedText());
      txtContent.replaceSelection("");
    } 
    else if (e.getActionCommand().equals(item[7])) { // copy
      toClipboard(txtContent.getSelectedText());
    } 
    else if (e.getActionCommand().equals(item[8])) { // paste
      txtContent.replaceSelection(fromClipboard());
    } 
    else if (e.getActionCommand().equals(item[9])) { // about
      JOptionPane.showMessageDialog(null, "JNLP ClipboardService", "About", JOptionPane.INFORMATION_MESSAGE);
    }
  }
  ...
  private void button_actionPerformed(ActionEvent e) {
    ...
    else if (e.getSource().equals(jbutton[4])) { // print
      print(txtContent);
    }
    else if (e.getSource().equals(jbutton[5])) { // page setup
      pageSetup();
    }
    else if (e.getSource().equals(jbutton[6])) { // cut
      toClipboard(txtContent.getSelectedText());
      txtContent.replaceSelection("");
    } 
    else if (e.getSource().equals(jbutton[7])) { // copy
      toClipboard(txtContent.getSelectedText());
    } 
    else if (e.getSource().equals(jbutton[8])) { // paste
      txtContent.replaceSelection(fromClipboard());
    } 
