...
public class JScrollPaneDemo extends JFrame implements CaretListener, DocumentListener, UndoableEditListener {
  ...
  HeaderView columnHeaderView;
  HeaderView rowHeaderView;
  ...
  public JScrollPaneDemo() {
    ...
    // 設定捲軸面板邊框之屬性值
    UIManager.put("ScrollPane.border", new javax.swing.border.EtchedBorder(EtchedBorder.LOWERED));
    
    // 設定文字標籤字型之屬性值
    UIManager.put("ScrollPane.font", new Font("dialog", Font.BOLD, 12));
    
    // 設定捲軸面板前景之屬性值
    UIManager.put("ScrollPane.foreground", Color.PINK);
    
    // 設定視界的邊框之屬性值
    UIManager.put("ScrollPane.viewportBorder", new javax.swing.border.EtchedBorder(EtchedBorder.RAISED));
    
    // 取得捲軸面板Pluggable Look and Feel屬性值
    System.out.println("JScrollPane Look and Feel: " + UIManager.getString("ScrollPaneUI"));
    ...
    columnHeaderView = new HeaderView(HeaderView.HORIZONTAL);
    rowHeaderView = new HeaderView(HeaderView.VERTICAL);

    columnHeaderView.setPreferredWidth((int)jtextarea.getPreferredSize().getWidth());
    rowHeaderView.setPreferredHeight((int)jtextarea.getPreferredSize().getHeight());

    JPanel cornerPanel = new JPanel(); 
    JLabel jlabel = new JLabel("CM");
    jlabel.setFont(new Font("Dialog", Font.PLAIN, 11));
    cornerPanel.add(jlabel); 
    ...
    // 設定視界的邊框
    jscrollpane.setViewportBorder(BorderFactory.createLineBorder(Color.RED));

    // 設定行標題的View Port
    jscrollpane.setColumnHeaderView(columnHeaderView);
    // 設定列標題的View Port
    jscrollpane.setRowHeaderView(rowHeaderView);

    // 設定View Port左上角的角落物件
    jscrollpane.setCorner(JScrollPane.UPPER_LEFT_CORNER,  cornerPanel);
    // 設定View Port左下角的角落物件
    jscrollpane.setCorner(JScrollPane.LOWER_LEFT_CORNER,  new Corner());
    // 設定View Port右上角的角落物件
    jscrollpane.setCorner(JScrollPane.UPPER_RIGHT_CORNER, new Corner());
